...
char *to_path(char *req) {
	...
	// For now, we don't support these requests. (We could always add support,
	// though!)
	if (last_slash == NULL) {
		return NULL;
	}
...
int handle_req(char *request, int socket_fd) {
	...
	if (path == NULL) {
		return respond_400(socket_fd, -1);
	}
	...
	if (fd == -1) {
		if (errno == ENOENT) {
			return respond_404(socket_fd, fd);
		} else {
	...
	// Write the header to the socket ("HTTP/1.1 200 OK" followed by a
	// Content-Type header)
	{
		char ext[4] = {0};

		{
			char *path_ext = strrchr(
				path, '.'); // Could skip the strrchr() if done in to_path
			size_t ext_len = strlen(path_ext);

			// If the extension is less than 4 bytes (5, counting the '.'), set
			// it to NULL.
		...
		char resp_str[1024]; // Our responses are short, e.g. "200
		...
		while (bytes_to_write) {
			bytes_written =
				write(socket_fd, resp_str + bytes_written, bytes_to_write);

			if (bytes_written == -1) {
				// If sending the 200 didn't succeed, the odds of 500 succeeding
				// aren't great!
				return respond_500(socket_fd, fd);
			}
...
// only easily test it on the one you're currently running).
#ifdef __linux__
			// 👉 Replace these hardcoded integers by calling the Linux
			// sendfile().
			//    Its docs are here:
			...
			// 👉 Replace these hardcoded integers by calling the macOS
			// sendfile().
			//    Its docs are here:
...
#error "Unsupported operating system"
			...
			if (send_failed) {
				// We already sent a 200 OK response, so it's too late to send a
				// 500.
				break;
			}
...
int main() {
	...
	while (1) {
		...
		int req_socket_fd = accept(socket_fd, (struct sockaddr *)&address,
								   (socklen_t *)&addrlen);
